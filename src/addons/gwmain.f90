...
subroutine gwmain
  ...
  real(8), allocatable :: vxcnk_allk(:,:,:)  !! for cumu
  real(8), allocatable :: vclnk(:,:,:)
  real(8), allocatable :: exxnk(:,:,:)
  real(8), allocatable :: exxnk_allk(:,:,:)  !! for cumu
  ...
  call genvq
  call genvgq
  ! read the density and potentials from file, if file exist?
  call readstate
  ...
  if (mpi_grid_root()) then
    ...
    write(151,'("MPI grid size                    : ",8I6)')(mpi_grid_dim_size(i),i=1,mpi_grid_nd)
    write(151,*)
    do i=1,nvq0
      write(151,'(" vqc : ",3G18.10)')vqc(:,i)
    enddo
    call flushifc(151)
    call system("mkdir -p Temp_files")    ! temp files folder
  endif
  ...
  if (mpi_grid_root()) write(*,*) " kq_mapping is done "
  ...
  call gen_gw_wmesh(0,emax,nw_se)                          !! nw_se=2 for caltype=0,1,3; nw_se=51 for caltype=2; WHY ??
  ...
  allocate(gw_self_energy_allk(nw_se,nbnd,nspinor,nkptnr)) !! for cumu
  allocate(se_aux2(nw_se,nbnd,nspinor,nkpt))
  allocate(vxcnk(nbnd,nspinor,nkptnrloc))
  allocate(vxcnk_allk(nbnd,nspinor,nkptnr))                !! for cumu
  allocate(vclnk(nbnd,nspinor,nkptnrloc))
  allocate(exxnk(nbnd,nspinor,nkptnrloc))
  allocate(exxnk_allk(nbnd,nspinor,nkptnr))                !! for cumu
  ...
  ! do gw band plot, no GW calculation needed. 
  if (gwband) then
    call gwband_dat(151)
...
202 continue
    ...
    if (mpi_grid_root((/dim_q/))) call write_exxvxc(1,nbnd,bndrg,exxnk,exxvc,vxcnk,vclnk)
  endif
    ...
    do isp1=1,nspinor
      ...
      do i=ibnd,fbnd
          if ((step0.eq.0).or.(step0.eq.1)) then
            prev_Eqp(i-ibnd+1,isp1,ik)=evalsvnr(i,ik)  ! E_KS
          elseif (step0.ge.2) then
            evalsvnr(i,ik)=prev_Eqp(i-ibnd+1,isp1,ik)   ! prev_Eqp is read from the file
          endif
      enddo !i
    enddo !isp1
  enddo !ik
  ...
  ! ----------------------------------------------------------------------------
  ! outer loop for GW0 calculations, 
  
  ! exx and vxc don't need to be updated when the system has a nonzero band gap
  ...
  do istep=1,scgwni 
    ...
    if (istep.le.(step0-1)) cycle
      ...
      do iqloc=1,nvqr
        ...
        write(fname,'("Temp_files/tmp_Sc_ik",I4.4,"_iq",I4.4)') mpi_grid_dim_pos(dim_k),mpi_grid_dim_pos(dim_q)
        ...
        if (mod(iqloc,2).eq.0) then !even
            write(fname,'("Temp_files/e_tmp_Sc_ik",I4.4,"_iq",I4.4)') mpi_grid_dim_pos(dim_k),mpi_grid_dim_pos(dim_q)
        else  !odd
            write(fname,'("Temp_files/o_tmp_Sc_ik",I4.4,"_iq",I4.4)') mpi_grid_dim_pos(dim_k),mpi_grid_dim_pos(dim_q)
        endif
      ...
      ! need complete corr_se_aux
      if (caltype.eq.3) then
        call mpi_grid_reduce(corr_se_aux(1,1,1,1,1),nw_se*nbnd*nspinor*nkptnr*nvqr,dims=(/dim_k,dim_q/),all=.true.)
      else
        call mpi_grid_reduce(corr_se_aux(1,1,1,1,1),nw_se*nbnd*nspinor*nkptnr*nvqr,dims=(/dim_k/),all=.true.)
      endif
      ...
      do ik1=1,nkpt
        ...
        do iq=1,nvqr
            do iqbz=1,iqrmap(2,iq)
              ...
              se_aux2(:,:,:,ik1) = se_aux2(:,:,:,ik1) + corr_se_aux(:,:,:,irk,iq)
            enddo
        enddo
        do ikloc=1,nkptnrloc
            ik = mpi_grid_map(nkptnr,dim_k,loc=ikloc)
      ...
      ! some factor for gw_self_energy
      ...
      ! total self-energy, adding exxnk
      do iw=1,nw_se
      ...
      call mpi_grid_reduce(Eqp(1,1,1),nbnd*nspinor*nkptnr,dims=(/dim_k/),all=.true.)
      ...
      if (scgwni.gt.1) then
        ...
        if (maxdel*ha2ev.le.0.01d0) then   !! why 0.01d0 ????
            ...
            exit ! this means converged, exit the GW0 loop
  ...
  ! -------------------------------------------------------------------
  ! G0W0 part, output files
  if ((caltype.eq.0).or.(caltype.eq.1).or.(caltype.eq.3)) then ! ppa,rai,new-rai, all 3 are G0W0
  ...
  if (mpi_grid_root()) then
    call timestamp(151)
    write(151,*) 
    write(151,'(" GW calculation is done. Now start Cumulant Expansion Approximation (CEA) calculation. ")')
    close(151)
  endif
  ...
  do ikloc=1,nkptnrloc
    ...
    do isp1=1,nspinor
      ...
      do ist=ibnd,fbnd
        i=ist-ibnd+1
        vxcnk_allk(i,isp1,ik) = vxcnk(i,isp1,ikloc)   
        exxnk_allk(i,isp1,ik) = exxnk(i,isp1,ikloc)
        do iw=1,nw_se
          gw_self_energy_allk(iw,i,isp1,ik) = gw_self_energy(iw,i,isp1,ikloc)
        enddo !iw
      enddo !ist
    enddo !isp1
  enddo !ikloc  
  call mpi_grid_reduce(vxcnk_allk(1,1,1), nbnd*nspinor*nkptnr, dims=(/dim_k/), all=.true.)
  call mpi_grid_reduce(exxnk_allk(1,1,1), nbnd*nspinor*nkptnr, dims=(/dim_k/), all=.true.)
  call mpi_grid_reduce(gw_self_energy_allk(1,1,1,1), nw_se*nbnd*nspinor*nkptnr, dims=(/dim_k/), all=.true.)

  
  ! ==============================================================================
  ! cumulant expansion approximation (CEA) correction to GWA, test implementation 
  ! 
  ! ready for use: 
  !                vxcnk_allk(nbnd,nspinor,nkptnr)          
  !                exxnk_allk(nbnd,nspinor,nkptnr)
  !                gw_self_energy_allk(nw_se,nbnd,nspinor,nkptnr)  
  
  if (mpi_grid_root().and.cumu) then
  
    write(*,*) "                               "
    write(*,*) " ----------------------------- "
    write(*,*) " do cumulant calculation here. "
    write(*,*) " ----------------------------- "
    write(*,*) "                               "    
  
  endif

  deallocate(vxcnk_allk)
  deallocate(exxnk_allk)
  deallocate(gw_self_energy_allk)
